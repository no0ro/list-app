class ListsController < ApplicationController
    ...
    def create 
      ...
      item = Item.find_or_create_by(name: params[:name]) # params[:items].map{|item| dont need to iterate bc will only ever be adding 1 item at a time, despite the items attribute is an array on teh List obj. when submit, its always just 1 item 
      # > Item.find_by(name: params[:name]) 
        # => [["name", "item4"]

    
      #byebug
      list.items << item # add item to the items array
      # > list.items << item 
